...
class Translator
  ...
  def translate(text, to="en", from='en' )
    begin
      ...
      # through post_response
      #uri = URI.parse(URI.encode("http://translate.google.com/translate_a/t".strip))
      
      #response = Net::HTTP.post_form(uri, {"client"=>"t","text" => text,"sl"=>"#{from.to_s.downcase}","tl"=>"#{to.to_s.downcase}","hl"=>"en","ie"=>"UTF-8","oe"=>"UTF-8","multires"=>1,"otf"=>1,"ssel"=>3,"tsel"=>3,"sc"=>1, "per_page" => "50"})
